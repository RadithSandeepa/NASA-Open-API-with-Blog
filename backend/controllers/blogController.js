...
//   const authorName = req.user.name || 'Unknown Author' ;
...
export const blogPost = catchAsyncErrors(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorHandler("Invalid file type. Only JPG, PNG, and WEBP Formats Are Allowed!", 400)
    );
  }
  ...
  if (!createdBy) {
    return next(new ErrorHandler("User data not properly initialized!", 500));
  }
